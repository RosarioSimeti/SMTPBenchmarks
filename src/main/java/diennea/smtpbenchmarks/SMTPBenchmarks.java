...
import diennea.smtpbenchmarks.collectors.ConsoleResultCollector;

public class SMTPBenchmarks {
		...
		try {
			...
			while (!checked) {
				if (args.length == 0) {
					...
					System.out.println("Type the useful commands below: ");
			...
			while (remaining > 0) {
				...
				messageIDSendTimesFutures.add(service.submit(new SendMessageTask(collector, host, port, username, password, session, message, messageCount, connectionIDGenerator, messageIDGenerator, messageIDHeader)));
			}
