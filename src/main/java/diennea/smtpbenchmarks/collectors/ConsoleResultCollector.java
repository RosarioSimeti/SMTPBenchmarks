package diennea.smtpbenchmarks.collectors;
...
import diennea.smtpbenchmarks.ResultCollector;
...
public class ConsoleResultCollector implements ResultCollector {
	...
	private static final String STANDARD_DEV_STRING = "    Standard deviation:  ";
	private static final String EVALUATION_STRING = "    Evaluation:          ";
	private static final String EVALUATION_NO_STRING = EVALUATION_STRING + "no evaluation";
	private static final String BLANKSPACES_STRING = "                         ";
	...
	public void finished() {
		...
		if (connectionCount.intValue() > 0) {
			...
			System.out.println(AVERAGE_STRING + format(format, totalSendTime, TimeUnit.NANOSECONDS, TimeUnit.MILLISECONDS) + " ms (on wall send time)");
			System.out.println(BLANKSPACES_STRING + format(format, connectionTime.getMean(), TimeUnit.NANOSECONDS, TimeUnit.MILLISECONDS) + " ms (on real connection time)");
			System.out.println(MINIMUM_STRING + format(format, connectionTime.getMin(), TimeUnit.NANOSECONDS, TimeUnit.MILLISECONDS) + " ms");
			System.out.println(MAXIMUM_STRING + format(format, connectionTime.getMax(), TimeUnit.NANOSECONDS, TimeUnit.MILLISECONDS) + " ms");
			System.out.println(STANDARD_DEV_STRING + format(format, connectionTime.getStandardDeviation(), TimeUnit.NANOSECONDS, TimeUnit.MILLISECONDS));
			...
			if (distribution) {
				...
				System.out.println(EVALUATION_STRING + format(format, connectionTime.getMean(), TimeUnit.NANOSECONDS, TimeUnit.MILLISECONDS) + " ms +-" + format(format, confidence, TimeUnit.NANOSECONDS, TimeUnit.MILLISECONDS) + " ms at " + (int) ((1.0 - significance) * 100) + "%");
			} else {
				System.out.println(EVALUATION_NO_STRING);
			}
		}
		...
		if (messageCount.intValue() > 0) {
			...
			System.out.println(AVERAGE_STRING + format(format, totalSendTime, TimeUnit.NANOSECONDS, TimeUnit.MILLISECONDS) + " ms (on wall send time)");
			System.out.println(BLANKSPACES_STRING + format(format, sendTime.getMean(), TimeUnit.NANOSECONDS, TimeUnit.MILLISECONDS) + " ms (on real send time)");
			System.out.println(MINIMUM_STRING + format(format, sendTime.getMin(), TimeUnit.NANOSECONDS, TimeUnit.MILLISECONDS) + " ms");
			System.out.println(MAXIMUM_STRING + format(format, sendTime.getMax(), TimeUnit.NANOSECONDS, TimeUnit.MILLISECONDS) + " ms");
			System.out.println(STANDARD_DEV_STRING + format(format, sendTime.getStandardDeviation(), TimeUnit.NANOSECONDS, TimeUnit.MILLISECONDS));
			...
			if (distribution) {
				...
				System.out.println(EVALUATION_STRING + format(format, sendTime.getMean(), TimeUnit.NANOSECONDS, TimeUnit.MILLISECONDS) + " ms +-" + format(format, confidence, TimeUnit.NANOSECONDS, TimeUnit.MILLISECONDS) + " ms at " + (int) ((1.0 - significance) * 100) + "%");
			} else {
				System.out.println(EVALUATION_NO_STRING);
			}

			System.out.println("\n  Message delivery speed");
			System.out.println(AVERAGE_STRING + formatEvent(format, messageCount.longValue(), totalSendTime, TimeUnit.NANOSECONDS, TimeUnit.SECONDS) + " msg/s (on wall send time)");
			System.out.println(BLANKSPACES_STRING + formatEvent(format, messageCount.longValue(), sendTime.getSum(), TimeUnit.NANOSECONDS, TimeUnit.SECONDS) + " msg/s (on real send time)");
		}
		...
		if (receive && receivedMessageCount.intValue() > 0) {
			{
				...
				System.out.println(AVERAGE_STRING + format(format, totalReceiveTime, TimeUnit.NANOSECONDS, TimeUnit.MILLISECONDS) + " ms (on wall round trip time)");
				System.out.println(BLANKSPACES_STRING + format(format, receiveTime.getMean(), TimeUnit.NANOSECONDS, TimeUnit.MILLISECONDS) + " ms (on real receive time)");
				System.out.println(MINIMUM_STRING + format(format, receiveTime.getMin(), TimeUnit.NANOSECONDS, TimeUnit.MILLISECONDS) + " ms");
				System.out.println(MAXIMUM_STRING + format(format, receiveTime.getMax(), TimeUnit.NANOSECONDS, TimeUnit.MILLISECONDS) + " ms");
				System.out.println(STANDARD_DEV_STRING + format(format, receiveTime.getStandardDeviation(), TimeUnit.NANOSECONDS, TimeUnit.MILLISECONDS));
				...
				if (distribution) {
					...
					System.out.println(EVALUATION_STRING + format(format, receiveTime.getMean(), TimeUnit.NANOSECONDS, TimeUnit.MILLISECONDS) + " ms +-" + format(format, confidence, TimeUnit.NANOSECONDS, TimeUnit.MILLISECONDS) + " ms at " + (int) ((1.0 - significance) * 100) + "%");
				} else {
					System.out.println(EVALUATION_NO_STRING);
				}

				System.out.println("\n  Message receive speed");
				System.out.println(AVERAGE_STRING + formatEvent(format, messageCount.longValue(), totalReceiveTime, TimeUnit.NANOSECONDS, TimeUnit.SECONDS) + " msg/s (on wall round trip time)");
				System.out.println(BLANKSPACES_STRING + formatEvent(format, messageCount.longValue(), receiveTime.getSum(), TimeUnit.NANOSECONDS, TimeUnit.SECONDS) + " msg/s (on real receive time)");
			}

			{
				...
				System.out.println(AVERAGE_STRING + format(format, totalReceiveTime, TimeUnit.NANOSECONDS, TimeUnit.MILLISECONDS) + " ms (on wall round trip time)");
				System.out.println(BLANKSPACES_STRING + format(format, sendAndReceiveTime.getMean(), TimeUnit.NANOSECONDS, TimeUnit.MILLISECONDS) + " ms (on real round trip time)");
				System.out.println(MINIMUM_STRING + format(format, sendAndReceiveTime.getMin(), TimeUnit.NANOSECONDS, TimeUnit.MILLISECONDS) + " ms");
				System.out.println(MAXIMUM_STRING + format(format, sendAndReceiveTime.getMax(), TimeUnit.NANOSECONDS, TimeUnit.MILLISECONDS) + " ms");
				System.out.println(STANDARD_DEV_STRING + format(format, sendAndReceiveTime.getStandardDeviation(), TimeUnit.NANOSECONDS, TimeUnit.MILLISECONDS));
				...
				if (distribution) {
					...
					System.out.println(EVALUATION_STRING + format(format, sendAndReceiveTime.getMean(), TimeUnit.NANOSECONDS, TimeUnit.MILLISECONDS) + " ms +-" + format(format, confidence, TimeUnit.NANOSECONDS, TimeUnit.MILLISECONDS) + " ms at " + (int) ((1.0 - significance) * 100) + "%");
				} else {
					System.out.println(EVALUATION_NO_STRING);
				}
				...
				System.out.println(AVERAGE_STRING + formatEvent(format, messageCount.longValue(), totalReceiveTime, TimeUnit.NANOSECONDS, TimeUnit.SECONDS) + " msg/s (on wall round trip time)");
				System.out.println(BLANKSPACES_STRING + formatEvent(format, messageCount.longValue(), sendAndReceiveTime.getSum(), TimeUnit.NANOSECONDS, TimeUnit.SECONDS) + " msg/s (on real round trip time)");
			}
		}
